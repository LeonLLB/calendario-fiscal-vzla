<template>
    <v-container @vnode-before-mount="()=>prepare()">
        <div class="text-center">
            AUTOLIQUIDACIÓN ANUAL DE IMPUESTO SOBRE LA RENTA (Ejercicio fiscal 01/01/{{ new Date().getFullYear()-1 }} al 31/12/{{ new Date().getFullYear()-1 }})
        </div>
            ...
            <v-table>
                ...
                <tbody>
                    ...
                    >
                        ...
                        <td style="text-align: center;" :class="getColorCode(fecha.fecha)">{{ formatDate(fecha.fecha) }}</td>
...
import LiquidacionForm from '../components/LiquidacionForm.vue';
...
import { FechaLiquidacion } from '../interfaces/Liquidaciones';

const hasConjuntos = ref(false)
const data: Ref<FechaLiquidacion[]> = ref([])
...
const prepare = () =>{
    ...
    .then(conjuntos=>{
        if(conjuntos){
            data.value = liquidacionController.defSeed(conjuntos)
            getFechas()
...
const getColorCode = (fecha:string): (string | undefined) => {
    if(fecha === 'N/A') return
    const actualDate = new Date()
    const dateToCompare = new Date(fecha.replaceAll('-','/'))
...
const onAssignClick = (fecha: FechaLiquidacion) => {
        fechaSeleccionada.value = fecha.fecha
        conjuntoRifSeleccionado.value = {id: fecha.conjuntoId,rif1:fecha.rif1,rif2:fecha.rif2}
        isDialogFormOpen.value = true
}

const getFechas = () => {
    liquidacionController.GetAll(conjuntos.value).then(res=>{  
...
const resetImpuestos = () => {
    liquidacionController.Delete().then(res=>{
        if(res.ok){
            prepare()
